...
st.set_page_config(page_title="Technology — Fifth Dragon Capital", layout="wide")
st.title("Technology")
...
def _stochrsi_panel(df: pd.DataFrame, x_fmt: str) -> alt.Chart | None:
    ...
    band_rules = (
        alt.Chart(bands).mark_rule(strokeDash=[3, 3], strokeWidth=1, opacity=0.5)
        .encode(
            y=alt.Y("level:Q"),
            color=alt.condition(
                alt.datum.level == 80,
...
def _chart(label: str, ticker: str, df: pd.DataFrame, prev_close: float | None, x_fmt: str, tooltip_fmt: str = "", alerts: list[dict] | None = None, show_ema: bool = False, indicator: str = "None", show_st: bool = False) -> None:
    ...
    ind_panel = None
    if indicator == "RSI":
        ind_panel = _rsi_panel(df, x_fmt)
    elif indicator == "StochRSI":
        ind_panel = _stochrsi_panel(df, x_fmt)
    elif indicator == "MACD":
        ind_panel = _macd_panel(df, x_fmt)
    elif indicator == "ADX":
        ind_panel = _adx_panel(df, x_fmt)
    ...
    summary = _signal_summary(df, show_ema, show_st, indicator)
    if summary:
...
if "last_period_p11" not in st.session_state:
    st.session_state.last_period_p11 = period_choice
if st.session_state.last_period_p11 != period_choice:
    st.cache_data.clear()
    st.session_state.last_period_p11 = period_choice


# ── technology panel (auto-refreshes for intraday) ────────────────────────────

@st.fragment(run_every=run_every)
def technology_panel() -> None:
    st.caption(
        f"Last updated: {datetime.now().strftime('%H:%M:%S')}  •  "
        f"Period: {period_choice}  •  "
        + ("Data delayed ~15 min" if period_choice == "Intraday" else "Daily OHLC bars")
    )
    if st.button("↺ Refresh now", key="tech_refresh"):
        st.cache_data.clear()
