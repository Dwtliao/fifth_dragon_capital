...
check_token_freshness() {
    if [[ ! -f "$TOKEN_FILE" ]]; then
        ...
        SYNC_TRIGGERED_BY=launchd "$PYTHON" -m etrade_sync log-event --job weekly_sync --status token_stale >> "$LOG_FILE" 2>&1 || true
...
if "$PYTHON" -m etrade_sync sync --days 30 >> "$LOG_FILE" 2>&1; then
    ...
    "$NOTIFY" -title "Fifth Dragon Capital" -subtitle "E*TRADE Pipeline" \
              -message "Weekly sync FAILED — check logs" -sound Basso
fi
