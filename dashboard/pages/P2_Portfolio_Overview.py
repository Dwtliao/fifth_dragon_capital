...
# Positions aggregated by symbol across accounts
positions_df = pd.DataFrame(query("""
    SELECT
        a.symbol,
        a.sector,
        a.asset_class,
...
delta_label = f"{float(daily_return):.2f}% today" if daily_return is not None else None
...
with col_l:
    ...
    if not sector_df.empty:
        st.altair_chart(
            donut_chart(sector_df, "market_value", "sector", "Sector"),
...
with col_r:
    ...
    if not asset_df.empty:
        st.altair_chart(
            donut_chart(asset_df, "market_value", "asset_class", "Asset Class"),
            use_container_width=True,
        )
...
if not positions_df.empty:
    ...
    # Format dollar and percentage columns for display
